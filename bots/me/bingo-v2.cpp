...
const int SCORE_COL_X_IN_A_ROW[ROWS + 1] = {1, 5, 20, 400, WIN_POINTS, 2 * WIN_POINTS, 3 * WIN_POINTS};
...
long long scoreRow[ROW_CONFIG], scoreCol[COL_CONFIG], scoreDiag[DIAGS][DIAG_CONFIG];
...
long long alphabeta(long long alpha, long long beta) {
    ...
    if (depth % 4 == 0 && checkTime() > TIME_LIMIT) {
        timeUp = true;
        return 0;
    }
