...
uint64_t compute_hash() {
    ...
    for (int i = 0; i < ROWS; ++i)
        for (int j = 0; j < COLS; ++j)
            ...
            else if (board[i][j] == 'O') h ^= zob[i][j][1];
    return h;
}
...
int eval_positional() {
    ...
    auto line_score = [&](int r, int c, int dr, int dc, char p) {
        int cnt = 0, blanks = 0, blocks = 0;
        for (int i = 0; i < 4; ++i) {
        ...
        if (cnt == 3 && blanks == 1) return 4000;
        if (cnt == 2 && blanks == 2) return 300;
        return 0;
    };
    ...
    for (int r = 0; r < ROWS; ++r) {
        for (int c = 0; c < COLS; ++c) {
            for (auto [dr, dc] : dirs) {
                score += line_score(r, c, dr, dc, ME);
                score -= line_score(r, c, dr, dc, OP);
            }
            ...
            if (board[r][c] == ME && r > 0 && board[r - 1][c] == '.')
                score -= 50;
            if (board[r][c] == OP && r > 0 && board[r - 1][c] == '.')
                score += 50;
        }
    }
...
int alphabeta(int depth, int alpha, int beta, bool maximizing, int ply) {
    ...
    // Check win before doing anything else
    if (depth == 0) return eval_positional();
...
int main() {
    ...
    for (int depth = 1; depth <= 42; ++depth) {
        ...
        for (int c : center_order) {
            ...
            int r;
            play(c, ME, r);
            int sc = check_win(r, c, ME) ? WIN_BASE / 1 : alphabeta(depth - 1, -INF, INF, false, 2);
            undo(c);
            if (timeout()) break;
            if (sc > best_score) {
                best_score = sc;
